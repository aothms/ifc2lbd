...
URI_LIST_PATH = "resources/ontologies/uri_list.txt"
DOWNLOAD_DIR = "resources/ontologies/downloaded"
SUCCESS_LOG = "resources/ontologies/scrap_success.log"
FAIL_LOG = "resources/ontologies/scrap_fail.log"

# Ensure download directory exists
os.makedirs(DOWNLOAD_DIR, exist_ok=True)

with open(URI_LIST_PATH, "r", encoding="utf-8") as f:
    uris = [line.strip().strip('"') for line in f if line.strip() and not line.strip().startswith('#')]
...
def extract_prefix_from_ttl(content):
    # Try to extract the prefix from the TTL content
    import re
...
def get_unique_path(filename, uri, prefix=None):
    # Use a 6-digit prefix (from URI hash or ontology prefix) and a 12-digit random hash
    if prefix:
        prefix6 = prefix[:6]
    else:
        prefix6 = hashlib.sha1(uri.encode('utf-8')).hexdigest()[:6]
    rand12 = secrets.token_hex(6)  # 12 hex digits
...
def try_download(url, uri_for_hash):
    try:
        ...
        prefix = None
        # Try to extract prefix if TTL or text
    ...
    def run():
        for candidate in candidates:
            ...
            if ok:
                ...
                return
        try:
            r = requests.get(uri, timeout=30, allow_redirects=True)
            r.raise_for_status()
...
for uri in uris:
    discover_and_download(uri, max_timeout=30)  # 60 seconds per URI
...
import csv
csv_path = "resources/ontologies/scrap_results.csv"
...
print(f"See {SUCCESS_LOG} and {FAIL_LOG} for details.")
